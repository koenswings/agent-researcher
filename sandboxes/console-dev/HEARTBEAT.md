...
## Check: Open PRs
- Are there open PRs on the `console-ui` repo that need attention?
- Has the Quality Manager left review comments that need a response?

## Check: Engine API changes
- Have there been recent commits to `engine/` that affect the API the Console depends on?
- If yes, flag for the next active session to assess impact.

## Check: Backlog
- Are there newly approved items in `hq/BACKLOG.md` under "Console UI"?
