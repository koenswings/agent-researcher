...
- [ ] Create `/home/pi/idea/` directory structure; confirm Docker volume mount
- [ ] Create org-root files: `CONTEXT.md`, `ROLES.md`, `PROCESS.md`, `BACKLOG.md`
- [ ] Create `prompting-guide-opus.md`
- [ ] Design standup template and enhance `./standup` script
- [ ] Write `scripts/export-backlog.sh`
- [ ] Create GitHub org (once name decided); transfer repos
- [ ] Create new agent workspace directories; initialise from GitHub
- [ ] Configure OpenClaw agents in `openclaw.json`
- [ ] Copy deployment files (IDENTITY, SOUL, USER, TOOLS, HEARTBEAT, BOOTSTRAP) to each agent
...
- [ ] Create `app-openclaw` repo structure (compose.yaml x-app block, init_data.tar.gz)
- [ ] Write `idea/openclaw/` config folder and `setup.sh`
