...
present with correct ids and workspace paths. Agents load lazily — no per-agent filesystem
entries until first session. No errors in startup logs. Brought back down.
...
context for browser pairing). MC containers can reach `http://172.18.0.1:18789` but not
`wss://openclaw-pi.tail2d60.ts.net:443` — Tailscale DNS doesn't resolve from inside Docker.
This is correct behaviour: the gateway connection is a UI task for the first live session.

**CEO live session todo (combine with steps 14+15):**
1. Start both stacks:
   - `cd /home/pi/openclaw && sudo docker compose up -d`
...
3. Settings → Gateways → Add:
   - URL: `wss://openclaw-pi.tail2d60.ts.net:443`
...
### Step 12b — Fix MC backend gateway connectivity + version reporting ✅
**Date:** 2026-03-01

**Problem:** Three sequential errors when registering the gateway in the MC UI:

1. `ssl=None` incompatibility (websockets v16 breaking change) — **already fixed** in a
   previous session. `gateway_rpc.py` now only passes the `ssl` kwarg when a custom
   (insecure) context is needed; `wss://` with secure mode lets websockets infer SSL from
   the URI scheme.

2. Network reachability — `extra_hosts` mapped `openclaw-pi.tail2d60.ts.net` to
   `100.102.15.46` (the Tailscale IP on the `tailscale0` TUN device inside the Tailscale
   container). This IP lives inside the Tailscale container's network namespace and is not
   routable from the MC containers.

**Fix 1 — Network reachability (`compose.yml`):**
- Changed `extra_hosts` for both `backend` and `webhook-worker` from
  `100.102.15.46` → `host-gateway`.
- `100.102.15.46` is the Tailscale IP on the `tailscale0` TUN device inside the
  Tailscale container's network namespace — unreachable from MC containers.
- `host-gateway` resolves to `172.19.0.1` (the Pi host's bridge IP on the MC network).
- Port 18789 confirmed reachable at `172.19.0.1:18789` (gateway publishes `0.0.0.0:18789`).

**Fix 2 — Gateway version reporting (`compose.yaml`):**
The gateway reported `version: "dev"` in its connect response because:
```javascript
version: process.env.OPENCLAW_VERSION ?? process.env.npm_package_version ?? "dev"
```
Neither env var was set. MC rejected `"dev"` as an unsupported version format.
Added `OPENCLAW_VERSION=2026.2.17` to the openclaw gateway environment (version sourced
from `/app/build-info.json` inside the container).

**Gateway URL for registration:**
`ws://openclaw-pi.tail2d60.ts.net:18789` (plain WS, port 18789).

**Gateway registration result:**
```json
{
...
```
Gateway successfully registered. MC backend can now reach and provision agents.

---

### Step 13 — Branch protection on `main` ✅
**Date:** 2026-02-27

All 6 previously private repos made **public** (IDEA is open-source by design; no secrets
were committed to any repo). Branch protection then applied to all 7 IDEA repos.

**Protection applied uniformly:**
- PRs required before merging (no direct pushes to `main`)
- `enforce_admins: true` — admin bypass disabled; the CEO must also use PRs
- Force pushes blocked
- Branch deletion blocked
- 0 required approving reviews — CEO can merge agent PRs without a separate review step

| Repo | Was | Now | Protection |
|------|-----|-----|-----------|
| `agent-engine-dev` | Public | Public | Updated (enforce_admins added) |
| `agent-researcher` | Private | **Public** | Applied |
| `agent-console-dev` | Private | **Public** | Applied |
| `agent-site-dev` | Private | **Public** | Applied |
| `agent-quality-manager` | Private | **Public** | Applied |
| `agent-programme-manager` | Private | **Public** | Applied |
| `app-openclaw` | Private | **Public** | Applied |

---

### Step 14 — Pair browser with each agent in OpenClaw UI
**Date:** 2026-03-01 (partial — gateway registered from CLI; browser pairing pending)

Gateway registration requires a browser on the Tailnet. The Mission Control backend
runs inside Docker and cannot resolve the Tailscale hostname, so this step cannot be
automated from the Pi.

**Pre-condition:** both stacks must be running.

```bash
# Start OpenClaw
cd /home/pi/openclaw && sudo docker compose up -d openclaw

# Start Mission Control
cd /home/pi/openclaw/mission-control && sudo docker compose -f compose.yml --env-file .env up -d
```

**Completed via CLI (2026-03-01):**

Gateway, board groups, and boards created via the MC REST API — no browser session needed.

| What | ID |
|------|----|
| Gateway "OpenClaw Pi" | `ccc8463b-8b7d-46a1-afea-f699081b44c2` |
| Group: Engineering | `85af1878-ce2c-4dcc-b49e-853a3d142d09` |
| Group: HQ | `afb9728f-561b-4a07-bc33-b84702263305` |
| Board: Engine Dev | `6bddb9d2-c06f-444d-8b18-b517aeaa6aa8` |
| Board: Console Dev | `ac508766-e9e3-48a0-b6a5-54c6ffcdc1a3` |
| Board: Site Dev | `7cc2a1cf-fa22-485f-b842-bb22cb758257` |
| Board: Quality Manager | `d0cfa49e-edcb-4a23-832b-c2ae2c99bf67` |
| Board: Programme Manager | `3f1be9c8-87e7-4a5d-9d3b-99756c35e3a9` |

**Still requires a live CEO browser session:**

4. Open `https://openclaw-pi.tail2d60.ts.net` → for each of the 5 new agents
   (engine-dev, console-dev, site-dev, quality-manager, programme-manager):
   - Click the agent tab
   - Pair the browser with that agent session

**Note:** `researcher` tab does not need browser pairing — Compass runs via
Claude Code CLI, not OpenClaw.

Once all 5 agents are paired, proceed immediately to Step 15 (BOOTSTRAP sessions)
...
- [x] **Step 12b:** Fix MC ↔ gateway connectivity (extra_hosts, OPENCLAW_VERSION); gateway registered
- [x] **Step 13:** Set up branch protection on `main` across all repos
- [x] **Step 14:** Gateway registered; board groups and boards created via API; all 5 agent sandboxes started via `chat.send` with `deliver=True`
- [x] **Step 15:** MC board lead agents provisioned via onboarding API; all 5 agents online in MC; SOUL.md updated with IDEA mission; BASE_URL fix applied
