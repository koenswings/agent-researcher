...
### Step 13 — Branch protection on `main` (partial)
**Date:** 2026-02-27

**Blocker:** GitHub Free plan does not support branch protection on **private** repos.
Protection requires either GitHub Pro (individual) or GitHub Team plan (org).

Six of the seven IDEA repos are private:
`agent-researcher`, `agent-console-dev`, `agent-site-dev`, `agent-quality-manager`,
`agent-programme-manager`, `app-openclaw`.

**What was applied:**

| Repo | Visibility | Action |
|------|-----------|--------|
| `agent-engine-dev` | Public | Updated: `enforce_admins=true`, `allow_force_pushes=false`, `allow_deletions=false`, PR required (0 reviews) |
| All others | Private | Blocked — GitHub Free plan |

**Protection settings on `agent-engine-dev`:**
- Direct pushes to `main` blocked (PRs required)
- Admin bypass disabled (`enforce_admins: true`) — even the repo owner must use a PR
- Force pushes blocked
- Branch deletion blocked
- 0 required approving reviews — the CEO can merge their own PRs (correct for solo operator)

**Path forward:**
When the GitHub org is created, move all repos there. GitHub Free for orgs supports
branch protection on public repos only. To protect private repos, upgrade to
**GitHub Team** ($4 USD/user/month). Recommended: apply the same ruleset to all repos
at org level once the org is set up.

**Workaround for now:** Operational convention — agents must always propose changes via PR;
CEO must not push directly to `main` on private repos.
